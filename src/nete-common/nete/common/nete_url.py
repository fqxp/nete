...
class NeteUrl:
    ...
    def from_string(cls, url):
        ...
        if connection_type in ('http', 'https'):
            return NeteUrl(ConnectionType.TCP, base_url=url)
        elif connection_type == 'local':
            return NeteUrl(ConnectionType.UNIX, socket_path=parsed_url.path)
        elif connection_type == 'http+ssh':
            return NeteUrl(ConnectionType.SSH,
    ...
    def __str__(self):
        return ('<NeteUrl connection_type={} base_url={} socket_path={} '
                'ssh_host={} ssh_port={} username={}>'.format(
...
class ConnectionTypeNotSupported(Exception):
    ...
    def __str__(self):
        return 'Connection type »{}« not supported'.format(
